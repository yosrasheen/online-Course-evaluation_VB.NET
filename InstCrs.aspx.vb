...
Partial Class TeaCrs
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        txtNo.Text = Session("iNo")
        txtName.Text = Session("iName")
    End Sub
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Cmd.Connection = SqlCon
        SqlCon.Open()
        Cmd.CommandText() = "Select * From InstructorCourse Where CourseCode='" & Trim(cmbCrs.SelectedItem.Text) & "' And Section='" & Trim(txtSection.Text) & "' And Year=" & Trim(txtYear.Text) & " And Semester='" & cmbSem.SelectedItem.Text & "'"
        Dr = Cmd.ExecuteReader
        If Dr.Read Then
            ClientScript.RegisterStartupScript(GetType(String), "", "<script>alert('Sorry, This Course is Already Assigned to Instructor');</script>")
            SqlCon.Close()
        Else
            SqlCon.Close()
            SqlCon.Open()
            Cmd.CommandText() = "Insert Into InstructorCourse Values('" & Trim(cmbCrs.SelectedItem.Text) & "','" & Trim(txtNo.Text) & "','" & Trim(txtSection.Text) & "'," & Trim(txtYear.Text) & ",'" & Trim(cmbSem.Text) & "')"
            Cmd.ExecuteNonQuery()
            SqlCon.Close()
            txtCrs.Text = ""
            txtSection.Text = ""
