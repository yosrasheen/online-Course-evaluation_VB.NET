...
Partial Class Instructor
    ...
    Public Sub NewData()
        ...
        If Session("CMD") = 1 Then
            ...
            SqlCon.Close()
        End If
        If Session("CMD") = 2 Then
            btnCrs.Visible = True
            SqlCon.Close()
            Cmd.Connection = SqlCon
            SqlCon.Open()
            Cmd.CommandText() = "Select * From [User] Where UserID='" & Session("InsID") & "'"
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Session("CMD") = 1 Then
            ...
            Else
                SqlCon.Close()
                SqlCon.Open()
                Cmd.CommandText() = "Insert Into [User] Values('" & Trim(txtNo.Text) & "','" & Trim(txtName.Text) & "','Instructor','" & Trim(txtPW1.Text) & "','" & Trim(txtEmail.Text) & "','Active')"
                Cmd.ExecuteNonQuery()
                SqlCon.Close()
                Call NewData()
                ClientScript.RegisterStartupScript(GetType(String), "", "<script>alert('The Instructor Account has been Successfully Added');</script>")
            End If
            SqlCon.Close()
        End If
        ...
        If Session("CMD") = 2 Then
            ...
            Cmd.ExecuteNonQuery()
            SqlCon.Close()
            ClientScript.RegisterStartupScript(GetType(String), "", "<script>alert('The Instructor Account has been Successfully Updated');</script>")
        End If
        SqlCon.Close()
    End Sub
    ...
    Protected Sub btnCrs_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCrs.Click
        Session("iNo") = txtNo.Text
        Session("iName") = txtName.Text
        Response.Redirect("InstCrs.aspx")
    End Sub
    ...
    Protected Sub Grid_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles Grid.ItemCommand
        ...
        Cmd.Connection = SqlCon
        SqlCon.Open()
        Cmd.CommandText() = "Delete From InstructorCourse Where InstructorID='" & Trim(txtNo.Text) & "' And CourseCode='" & e.Item.Cells(1).Text & "' And Section='" & e.Item.Cells(3).Text & "' And Year=" & Session("Year") & " And Semester='" & Session("Sem") & "'"
        Cmd.ExecuteNonQuery()
        SqlCon.Close()
        Call FillGrid()
        Call FillGrid1()
    ...
    Sub FillGrid()
        SqlCon.Close()
        SqlCon.Open()
        Cmd.CommandText() = "Select InstructorCourse.CourseCode 'Course Code', CourseName 'Course Name', Section From InstructorCourse,Course Where InstructorCourse.CourseCode=Course.CourseCode and InstructorID='" & Trim(txtNo.Text) & "' And Year=" & Session("Year") & " And Semester='" & Session("Sem") & "'"
        Dr = Cmd.ExecuteReader
        If Dr.HasRows Then
            Grid.Visible = True
    ...
    Sub FillGrid1()
        SqlCon.Close()
        SqlCon.Open()
        Cmd.CommandText() = "Select Year, Semester From InstructorCourse Where InstructorID='" & Trim(txtNo.Text) & "' Group By Year, Semester"
        Dr = Cmd.ExecuteReader
        If Dr.HasRows Then
            Grid1.Visible = True
            lblErr.Visible = False
